...
function FileUploaderApp() {
  ...
  // Uses the browser's native fetch + FormData. The server streams it to LibreTime
  // without buffering, so 288MB show files aren't a problem.
  const handleFilePick = useCallback(async () => {
    ...
    try {
      ...
      // Required by LibreTime — all derived from the File object
      ...
      // /upload returns the raw LibreTime file object (LibreFileSchema shape)
      const data = await response.json() as LibreFile
      setUploadedFile(data)
      setUploadState('success')

      // Inform the LLM what was uploaded so it can reference it in follow-up turns
  ...
  }, [file, uploadUrl, uploadToken, metadata, app])

  // URL INPUT MODE: call the upload_file tool directly from the UI.
  // The server fetches the URL and uploads to LibreTime server-side.
  // app.callServerTool() returns the result directly — it doesn't re-trigger ontoolresult.
  ...
  return (
    <div className={styles.container}>
      ...
      {/* Metadata fields — same in both modes */}
...
// Mount the React app into the <div id="root"> in mcp-app.html
